;;extension stats use in em algorithm (binomial distribution) 
extensions[stats]
...
;;properties of agent
workers-own[
  answer           ;answer 0 = flood, 1 = normal
  workers-type     ;type 0 = normal, type 1 = fixed adversaries, type 2 = random adversaries , type 3 = perfect adversaries 
  penalty          ;
  penalty-all 
  delay-tick 
  recent-road 
...
to setup-flood 
  ...
  if problem-label = "random-seed"
  [ 
    random-seed new-seed with-local-randomness [
      random-seed ticks
...
to generate-workers[number w-type]
  ...
  [
    ...
    set recent-road []
    set available-road n-values (area.x * area.y) [? + 1]
...
to warp-forward
  let this-road [road-num] of patch xcor ycor
  let next-move random (area.x * area.y) + 1
  if length available-road > 0
  [set next-move one-of available-road]
  move-to one-of patches with [road-num = next-move]
end
...
to workers-send[who-worker]
    ...
    ifelse pcolor != grass-color and (not member? road-num recent-road or delay-tick <= ticks) and ticks > 0 and count-send < max-count-send
    [
      workers-ans
      ...
      set recent-road lput road-num recent-road
      set available-road remove road-num available-road

      ask out-link-to datacenter road-num[show-link]
      
      set count-send count-send + 1
    ]
...
to datacenter-fusion
   ...
   ifelse round-cut-type = 1[
    if use-cut-worker-by-penalty? and floor(ticks / update-ticks) >= start-cut and floor(ticks / update-ticks) < end-cut
      [add-kill-list]
       kill-list
   ]
   [
...
to assign-penalty
  ifelse reputation-algorithm = "None"
  [set use-cut-worker-by-penalty?  false]
  [set use-cut-worker-by-penalty?  true]
...
to add-adversaries
  ...
  [set first-number-workers number-workers]
  
  let plus-workers 5
  
  generate-workers(plus-workers)(adversaries-type)
  set number-workers number-workers + plus-workers
  
  let i 0
  while [i < 5]
  [set em-list lput [] em-list 
   set i i + 1]
  
  if ticks = end-ticks
  [set number-workers first-number-workers]
end
...
problem-label
problem-label
"flood" "normal" "random" "random-seed" "real"
2
...
179
normal
count workers with [workers-type = 0 and color != red]
17
1
11
...
205
precision
length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]
2
1
11
...
206
recall
length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [workers-type != 0]
2
1
11
...
<experiments>
  ...
  <experiment name="mvsoft2" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="0" step="1" last="100"/>
    <enumeratedValueSet variable="start-cut">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="number-workers">
      <value value="100"/>
  ...
  <experiment name="mvsoft3" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="0" step="1" last="100"/>
    ...
    <enumeratedValueSet variable="max-count-send">
      <value value="1"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="mvsoft11" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="0" step="1" last="100"/>
  ...
  <experiment name="mvsoft12" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="0" step="1" last="100"/>
  ...
  <experiment name="mvsoft3cut" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="0" step="1" last="100"/>
  ...
  <experiment name="mvsoft2cut" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="0" step="1" last="100"/>
  ...
  <experiment name="mvsoft11cut" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="0" step="1" last="100"/>
  ...
  <experiment name="mvsoft12cut" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="0" step="1" last="100"/>
  ...
  <experiment name="mvsoft3recall" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [workers-type != 0]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
    <enumeratedValueSet variable="start-cut">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="number-workers">
  ...
  <experiment name="mvsoft2ratio" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctkill / (floor(ticks / update-ticks) - start-cut)</metric>
    <steppedValueSet variable="adversaries-rate" first="0" step="1" last="100"/>
    <enumeratedValueSet variable="start-cut">
      <value value="5"/>
    </enumeratedValueSet>
  ...
  <experiment name="emfix2" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="0" step="1" last="100"/>
  ...
  <experiment name="mvsoft3precision" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
    <enumeratedValueSet variable="start-cut">
      <value value="5"/>
    </enumeratedValueSet>
  ...
  <experiment name="mvsoft3dday1" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="2" step="2" last="12"/>
    <enumeratedValueSet variable="adversaries-rate">
      <value value="15"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="start-cut">
  ...
  <experiment name="mv3dday1" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="14" step="2" last="16"/>
    <enumeratedValueSet variable="adversaries-rate">
      <value value="15"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="start-cut">
  ...
  <experiment name="mvhard3dday1pilus" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="14" step="2" last="16"/>
  ...
  <experiment name="mvsoft3dday2" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="15" step="5" last="40"/>
    <enumeratedValueSet variable="max-count-send">
      <value value="5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="start-cut">
  ...
  <experiment name="emsoft3dday2" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="15" step="5" last="40"/>
  ...
  <experiment name="mv2dday3" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="10"/>
    <enumeratedValueSet variable="adversaries-rate">
      <value value="15"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="start-cut">
  ...
  <experiment name="mvsoft3dday3" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="10"/>
  ...
  <experiment name="emsoft3dday3" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="10"/>
  ...
  <experiment name="emsoft3dday1" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="2" step="2" last="12"/>
  ...
  <experiment name="em3dday1" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="2" step="2" last="12"/>
  ...
  <experiment name="mvsoft3dday1plus" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="14" step="2" last="16"/>
  ...
  <experiment name="emsoft3dday1plus" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="14" step="2" last="16"/>
  ...
  <experiment name="emhard3dday1" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="2" step="2" last="16"/>
    <enumeratedValueSet variable="adversaries-rate">
      <value value="15"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="start-cut">
  ...
  <experiment name="mvsoft2dday2" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="15" step="5" last="40"/>
  ...
  <experiment name="mvhard11dday2" repetitions="50" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="15" step="5" last="40"/>
  ...
  <experiment name="mvsoft11dday2" repetitions="50" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="15" step="5" last="40"/>
  ...
  <experiment name="mvsoft11recall" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [workers-type != 0]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvsoft12recall" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [workers-type != 0]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvhard3recall" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [workers-type != 0]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvhard2precision" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="emsoft3recall" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [workers-type != 0]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="emsoft11recall" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [workers-type != 0]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="emsoft12recall" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [workers-type != 0]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="emsoft2recall" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [workers-type != 0]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="emsoft2precision" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="emsoft11precision" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="emsoft12precision" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="emsoft3precision" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvhard11recall" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [workers-type != 0]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvhard12recall" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [workers-type != 0]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvsoft2restartcut" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="start-cut" first="1" step="1" last="33"/>
    <enumeratedValueSet variable="adversaries-rate">
      <value value="15"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="number-workers">
  ...
  <experiment name="mvsoft3restartcut" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="start-cut" first="1" step="1" last="33"/>
  ...
  <experiment name="mvsoft11restartcut" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="start-cut" first="1" step="1" last="33"/>
  ...
  <experiment name="mvsoft2precision" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>length filter [? != 0] [workers-type] of workers with [color = red] / count workers with [color = red]</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvsoft3morecut" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="cut-workers-per-round" first="1" step="1" last="9"/>
    <enumeratedValueSet variable="start-cut">
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="adversaries-rate">
  ...
  <experiment name="mvsoft2morecut" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="cut-workers-per-round" first="1" step="1" last="9"/>
  ...
  <experiment name="mvsoft11morecut" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="cut-workers-per-round" first="1" step="1" last="9"/>
  ...
  <experiment name="mvsoft12morecut" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="cut-workers-per-round" first="1" step="1" last="9"/>
  ...
  <experiment name="mv3area" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="5"/>
    <enumeratedValueSet variable="adversaries-rate">
      <value value="25"/>
      <value value="50"/>
      <value value="75"/>
  ...
  <experiment name="mv11area" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="5"/>
  ...
  <experiment name="mvsoft11area" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="5"/>
  ...
  <experiment name="mvsoft12area" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="5"/>
  ...
  <experiment name="mvsoft2area" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="5"/>
  ...
  <experiment name="mvsoft3area" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="5"/>
  ...
  <experiment name="mvhard3area" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="5"/>
  ...
  <experiment name="mvhard2area" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="5"/>
  ...
  <experiment name="mvhard11area" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="5"/>
  ...
  <experiment name="mvhard12area" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="max-count-send" first="1" step="1" last="5"/>
  ...
  <experiment name="mv3arearate" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mv2arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mv11arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mv12arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvsoft3arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvsoft2arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvsoft11arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvsoft12arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvhard11arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvhard12arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvhard2arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="mvhard3arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="em3arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="em11arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="em12arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
  ...
  <experiment name="emsoft12arearate" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <steppedValueSet variable="adversaries-rate" first="1" step="1" last="100"/>
