...
datacenters-own[list-answer all-list-answer all-list-workers list-workers generate-label each-correctness]
...
to save-data
  ask datacenters[ 
    set all-list-answer lput list-answer all-list-answer
    set all-list-workers lput list-workers all-list-workers
  ]
end
...
to weight-mv[parameter]
   ...
   ifelse sum one-list > sum zero-list 
...
;   let one-answer count honesties with [answer = 1] + count adversaries with [answer = 1]
;   let zero-answer count honesties with [answer = 0] + count adversaries with [answer = 0]

;   if answer = 1
;   [set penalty (penalty + 1 / one-answer)]
;   if answer = 0
;   [set penalty (penalty + 1 / zero-answer)]
end
...
to cut-list
  ...
  while [i <= (area.x * area.y) + number-workers][
   ...
   let list-workers-cut sublist order(list-penalty) 0 (round(cut-workers-per-round / 100 * number-workers))
   foreach list-workers-cut [
     ask datacenters[
       let index position ? list-workers
       if index != false[
...
<experiments>
  ...
  <experiment name="test5" repetitions="10" runMetricsEveryStep="false">
    ...
    <steppedValueSet variable="cut-workers-per-round" first="0" step="1" last="50"/>
    <enumeratedValueSet variable="show-penalty?">
      <value value="false"/>
    </enumeratedValueSet>
  </experiment>
  ...
  <experiment name="test16" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>sum-correctness / floor(ticks / update-ticks)</metric>
    <enumeratedValueSet variable="workers-probability">
